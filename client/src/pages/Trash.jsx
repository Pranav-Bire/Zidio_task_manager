    ...
    try {
        ...
        switch (type) {
            ...
            case "deleteAll":
                ...
                const trashedTasks = data?.tasks?.filter(task => task.isTrashed);
                if (trashedTasks?.length > 0) {
                    await Promise.all(trashedTasks.map(task => 
                        deleteRestoreTask({
                            id: task._id,
                            actionType: "delete"
                        })
                    ));
                    result = { message: "All tasks permanently deleted" };
                } else {
                    throw new Error("No trashed tasks found");
                }
                break;
            ...
            case "restoreAll":
                // Get all trashed tasks
                const allTrashedTasks = data?.tasks?.filter(task => task.isTrashed);
                if (allTrashedTasks?.length > 0) {
                    await Promise.all(allTrashedTasks.map(task => 
                        deleteRestoreTask({
                            id: task._id,
                            actionType: "restore"
                        })
                    ));
                    result = { message: "All tasks restored successfully" };
                } else {
                    throw new Error("No trashed tasks found");
                }
                break;
